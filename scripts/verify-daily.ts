...
assert.equal(game.rerollsLeft, 3);
...
assert.deepEqual(dailyAttempt(game.challenge.date, 'current'), { attempt: 1, date: game.challenge.date, hardMode: false, era: 'current', challenge: game.challenge, complete: true, ...outcome });
...
assert.match(dailyShareText(game.challenge, 0, false), /CHALLENGE FAILED/);
assert.match(dailyShareText(game.challenge, game.challenge.target + 1, true), /CHALLENGE PASSED/);
...
game.abandonRun();
useGame.getState().startDaily({ era: 'current' });
assert.equal(useGame.getState().phase, 'setup', 'Second Normal attempt is blocked');
useGame.getState().startDaily({ era: 'current', hardMode: true });
assert.equal(useGame.getState().phase, 'ready', 'Hard has its own attempt');
assert.equal(useGame.getState().hardMode, true);
assert.equal(useGame.getState().rerollsLeft, 0);
assert.notEqual(useGame.getState().seed, normalSeed);
useGame.getState().abandonRun();
assert.equal(dailyAttempts(dailyChallenge().date, 'current').length, 1);
assert.equal(dailyAttempts(dailyChallenge().date, 'current', true).length, 1);
assert.equal(dailyAttempt(dailyChallenge().date, 'current')?.score, outcome.score);
useGame.getState().startDaily({ era: 'current', hardMode: true });
assert.equal(useGame.getState().phase, 'setup', 'Second Hard attempt is blocked after quitting');
...
useGame.getState().startDaily({ era: 'alltime' });
assert.equal(useGame.getState().phase, 'setup', 'Second All-Time Normal attempt is blocked');
useGame.getState().startDaily({ era: 'alltime', hardMode: true });
assert.equal(useGame.getState().phase, 'ready', 'All-Time Hard has its own attempt');
useGame.getState().abandonRun();
useGame.getState().startDaily({ era: 'alltime' });
assert.equal(useGame.getState().phase, 'setup', 'Third All-Time attempt is blocked');
...
assert.deepEqual(dailyStats(new Date(2026, 8, 22)), { current: 0, best: 1, played: 1, wins: 1 });
...
assert.equal(dailyStats(new Date(2026, 8, 22)).current, 3, 'Consecutive winning dates count once across modes');
