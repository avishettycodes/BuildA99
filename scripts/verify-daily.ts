...
game.abandonRun();
useGame.getState().startDaily({ era: 'current' });
assert.equal(useGame.getState().phase, 'ready', 'Second Current attempt is available');
assert.equal(useGame.getState().challenge?.attempt, 2);
assert.notEqual(useGame.getState().seed, normalSeed);
useGame.getState().abandonRun();
assert.equal(dailyAttempts(dailyChallenge().date, 'current').length, 2);
assert.equal(dailyAttempts(dailyChallenge().date, 'current')[0].score, outcome.score, 'Retry keeps the first result');
useGame.getState().startDaily({ era: 'current' });
assert.equal(useGame.getState().phase, 'setup', 'Third Current attempt is blocked');
...
useGame.getState().startDaily({ era: 'alltime' });
assert.equal(useGame.getState().phase, 'ready', 'Second All-Time attempt is independent');
useGame.getState().abandonRun();
useGame.getState().startDaily({ era: 'alltime' });
