...
import { dailyAttempt, dailyChallenge, dailyGoal, dailyOutcome } from '../lib/daily';
...
export function DailyCard({ onStart, canResume }: { onStart: () => void; canResume: boolean }) {
  ...
  const seconds = 86400 - (now.getUTCHours() * 3600 + now.getUTCMinutes() * 60 + now.getUTCSeconds());
  ...
  return (
    <section className="mb-8 rounded-xl border-2 border-hazard/60 bg-hazard/10 p-5">
      ...
      <p className="mt-2 font-mono text-[11px] text-white/55">All-time · Hard mode · One attempt per day on this browser. Same spins for everyone. Quitting uses your attempt.</p>
      {attempt ? (
      ...
      <p className="mt-3 font-mono text-[11px] text-white/45">Next challenge in {countdown} · Resets at midnight UTC</p>
    </section>
  );
}
